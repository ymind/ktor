...
class PathPatternSerializationTest {
    ...
    @Test
    fun testSimplePath() {
        val pathPattern = locationsFormat.encodeToPathPattern(SimplePath.serializer(), SimplePath())
    ...
    @Test
    fun testNestedPath() {
        val pathPattern = locationsFormat.encodeToPathPattern(
            NestedClass.ChildClass.serializer(),
            NestedClass.ChildClass(NestedClass())
        )
    ...
    fun testNestedPathDoesNotDoubleSlash() {
        val pathPattern1 = locationsFormat.encodeToPathPattern(
            ...
            NestedClassWithSlash.ChildClassWithSlash(NestedClass())
        )
        ...
        val pathPattern3 = locationsFormat.encodeToPathPattern(
            ...
            NestedClassWithSlash.ChildClassWithoutSlash(NestedClass())
        )
        ...
        val pathPattern2 = locationsFormat.encodeToPathPattern(
            ...
            NestedClassWithoutSlash.ChildClassWithSlash(NestedClass())
        )
    ...
    fun testMultipleNestedParentsShouldThrow() {
        assertFailsWith<IllegalArgumentException> {
            locationsFormat.encodeToPathPattern(
                Container.serializer(),
                Container(MultipleParents(NestedClass(), "", NestedClassWithSlash()))
            )
        }.let {
            assertEquals(
